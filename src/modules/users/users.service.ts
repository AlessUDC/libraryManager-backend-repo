...
export class UsersService {
    ...
    if (search) {
      if (filterBy === 'name') {
        where.OR = [
          ...
          { userData: { paternalSurname: { contains: search, mode: 'insensitive' } } },
          { userData: { maternalSurname: { contains: search, mode: 'insensitive' } } },
        ];
        ...
        where.OR = [
          ...
          { userData: { documentNumber: { contains: search, mode: 'insensitive' } } },
        ];
  ...
  async createByAdmin(dto: any) {
    const { 
      email, code, documentNumber, role, birthdate, 
      documentType, schoolId, facultyId, cycle, address, provinceId,
      ...userDataRest 
    } = dto;
    ...
    const slug = await this.generateUniqueSlug(`${userDataRest.name} ${userDataRest.paternalSurname} ${code}`);
    ...
    try {
      ...
      return await this.prisma.$transaction(async (tx) => {
        ...
        if (role === Role.STUDENT) {
            ...
            await tx.student.create({ data: { userId: user.userId, schoolId, cycle: dto.cycle || 1 } });
            ...
            await tx.librarian.create({ data: { userId: user.userId, shift: dto.shift || 'MAÑANA' } });
        } else if (role === Role.ADMINISTRATOR) {
            await tx.administrator.create({ data: { userId: user.userId } });
        }

        // Envío de correo de activación
        await this.mailService.sendActivationEmail(email, userData.name, code, activationToken);

        return {
          message: 'Usuario creado exitosamente. Se ha enviado un enlace de activación.',
          userId: user.userId,
          slug: user.slug,
          code: user.code,
      ...
      console.error(error);
      throw new InternalServerErrorException('Error al crear el usuario desde administración');
    }
  }

  async createAccount(dto: CreateAccountDto) {
    const { 
      email, code, documentNumber, documentType, role, schoolId, facultyId, 
    ...
    try {
      ...
      return await this.prisma.$transaction(async (tx) => {
        ...
        const placeholderPassword = await bcrypt.hash(uuid(), 10);
        const { token: confirmationToken, expiration: tokenExpiration } = this.generateConfirmationToken(24);
        ...
        // Envío de correo de activación
        await this.mailService.sendActivationEmail(email, userData.name, code, confirmationToken);
  ...
  async updateUserAdmin(userId: string, dto: UpdateUserDto) {
    const { role, isConfirmed, name, paternalSurname, maternalSurname, email, code, ...rest } = dto;
    ...
    if (code || email || dto.documentNumber) {
      ...
      if (email && email !== userToUpdate.userData.email) orConditions.push({ userData: { email } });
      if (dto.documentNumber && dto.documentNumber !== userToUpdate.userData.documentNumber) {
      ...
      if (orConditions.length > 0) {
        ...
        if (existing) {
          throw new ConflictException('El código, email o número de documento ya están en uso por otro usuario');
        }
      }
    }

    const currentName = name || userToUpdate.userData?.name || '';
    const currentPaternal = paternalSurname || userToUpdate.userData?.paternalSurname || '';
    const currentCode = code || userToUpdate.code;
    
    let newSlug = undefined;
    if (name || paternalSurname || code) {
      newSlug = await this.generateUniqueSlug(`${currentName} ${currentPaternal} ${currentCode}`);
    }

    const { birthdate, schoolId, facultyId, cycle, address, provinceId, documentType, ...userDataRest } = rest;
    ...
    return await this.prisma.$transaction(async (tx) => {
      ...
      if (address) {
        if (addressId) {
          await tx.address.update({ where: { addressId }, data: { title: address } });
        } else {
          const newAddress = await tx.address.create({ data: { title: address } });
